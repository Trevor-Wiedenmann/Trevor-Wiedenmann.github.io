...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	*/
	...
	if (!empty($scrubbed['name']) && !empty($scrubbed['email']) && !empty($scrubbed['comments']) ) {
		...
		// Send the email:
		mail('trevor.wiedenmann@mtchs.org', 'Contact Form Submission', $body, "From: {$scrubbed['email']}");
		// Print a message:
		echo'<p><em>Thank you for contacting me. I will reply some day.</em></p>';
		// Cleas $_POST (so that the form's not sticky):
		$_POST = array();
		} else {
...
?>
    ...
    <p>Name: <input type="text" name="name"="30"maxlength="60" value="<?php if (isset($scrubbed['name'])) echo $scrubbed['name']; ?>" /></p>
    <p>Email Address: <input type="text" name="email"="30"maxlength="80" value="<?php if (isset($scrubbed['email'])) echo $scrubbed['email']; ?>" /></p>
