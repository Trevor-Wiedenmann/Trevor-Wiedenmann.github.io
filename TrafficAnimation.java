...
public class TrafficAnimation extends JPanel
{
	...
	public void paintComponent(Graphics g)
	{
		...
		g.fillRect(xOffset + 133, squareY, squareSide, squareSide);
		
		int newSquare = height/3;
		int newSquareY = height/2 - newSquare/2;
		g.setColor(Color.RED);
		g.fillRect(xOffset, newSquareY, newSquare, newSquare);

		g.setFont(new Font("Helvetica", Font.BOLD, height/10));
		g.drawString("Hi.", width/2, height/5);
		
		int circleDiameter = newSquare / 2;
		int circleY = height/2 - circleDiameter/2;
		g.setColor(Color.WHITE);
		g.fillOval(xOffset + 25, circleY, circleDiameter, circleDiameter);

		int newCircle = newSquare / 10;
		int newCircleY = height/2 - newCircle/2;
		g.setColor(Color.BLUE);
		g.fillOval(xOffset + 35, newCircleY - 15, newCircle, newCircle);
		
		int newCircle2 = newSquare / 10;
		int newCircle2Y = height/2 - newCircle2/2;
		g.setColor(Color.ORANGE);
		g.fillOval(xOffset + 35, newCircle2Y + 15, newCircle2, newCircle2);
		
		int newSquare2 = height/20;
		int newSquare2Y = height/2 - newSquare2/2;
		g.setColor(Color.MAGENTA);
		g.fillRect(xOffset + 65, newSquare2Y, newSquare2, newSquare2);
	}
